...
class BaseWorkflow(object, metaclass=abc.ABCMeta):
    ...
    def get_node_and_edge_trace_for_ploty_network(self, G, pos, node_size=20, node_color='rgb(146, 237, 5)'):
    ...
    def draw_plotly_network(self, G=None, pos=None, title='Workflow', node_size=20, node_color='rgb(146, 237, 5)',save_fig_path=''):
        G = G if G is not None else self.get_networkx_graph()
        pos = pos if pos is not None else nx.spring_layout(G)
