...
class BaseResource(object, metaclass=abc.ABCMeta):
    ...
    def __init__(self, name:str, team_id:str, cost_per_time=None, workamount_skill_mean_map=None, workamount_skill_sd_map=None, quality_skill_mean_map=None):
        
        # Constraint variables on simulation
        self.name = name
        self.team_id = team_id
        self.cost_per_time = cost_per_time if cost_per_time is not None else 0.0
        self.workamount_skill_mean_map = workamount_skill_mean_map if workamount_skill_mean_map is not None else {}
        self.workamount_skill_sd_map = workamount_skill_sd_map if workamount_skill_sd_map is not None else {}
        self.quality_skill_mean_map = quality_skill_mean_map if quality_skill_mean_map is not None else {}
    ...
    def get_work_amount_skill_point(self, task_name):
        ...
        sum_of_working_task_in_this_time = sum(map(lambda task:task.state == BaseTaskState.WORKING, self.assigned_task_list))
