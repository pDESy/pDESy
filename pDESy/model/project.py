...
class Project(BaseProject):
    ...
    def read_pdesy_web_json(self, file_path: str, encoding=None):
        ...
        component_list = [
            Component(cd["name"], cd["id"], float(cd["userData"]["errorTolerance"]))
            for cd in cd_list
        ]
        ...
        task_list = [
            Task(
                td["name"],
                td["id"],
                float(td["userData"]["workAmount"]),
                float(td["userData"]["progress"]),
                float(td["userData"]["additionalWorkAmount"]),
            )
            for td in td_list
        ]
    ...
    def read_json(self, file_path: str, encoding=None):
        ...
        component_list = [
            Component(cd["Name"], cd["-id"], float(cd["ErrorTolerance"]))
            for cd in cd_list
        ]
