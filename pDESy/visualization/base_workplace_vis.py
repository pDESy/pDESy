#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Visualization helpers for BaseWorkplace."""
...
def create_simple_gantt(
    ...
    target_instance_list = workplace.facility_set
    if target_id_order_list is not None:
        id_to_instance = {instance.ID: instance for instance in workplace.facility_set}
    ...
    if print_workplace_name:
        y_tick_labels = [
            f"{workplace.name}: {facility.name}" for facility in target_instance_list
        ]

    gnt.set_yticks(y_ticks)
    gnt.set_yticklabels(y_tick_labels)

    for time, facility in enumerate(target_instance_list):
...
def create_data_for_gantt_plotly(
    ...
    for facility in target_instance_list:
        ...
        if print_workplace_name:
            task_name = f"{workplace.name}: {facility.name}"
...
def create_cost_history_plotly(
    ...
    data = create_data_for_cost_history_plotly(
        workplace, init_datetime, unit_timedelta
    )
    fig = go.Figure(data)
